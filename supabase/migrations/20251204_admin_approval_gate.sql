...
BEGIN
  ...
  IF new.raw_user_meta_data->>'role' = 'supplier' THEN
    -- Using 'pending' as initial status. Adjust if 'pending_payment' is preferred, but 'pending' is generic.
    INSERT INTO public.suppliers (id, contact_email, subscription_status, role)
    VALUES (new.id, new.email, 'pending', 'supplier');
  ELSIF new.raw_user_meta_data->>'role' = 'agent' THEN
    INSERT INTO public.agent_profiles (id, email, verification_status, role)
    VALUES (new.id, new.email, 'pending', 'agent');
  END IF;
